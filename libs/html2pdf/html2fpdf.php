...
function OpenTag($tag,$attr)
{
	...
	switch($tag) {
		...
		case 'TD':
			...
			{
				$v = $properties['FONT-SIZE'];
				if(is_numeric($v{0}))
				{
					$mmsize = ConvertSize($v,$this->pgwidth);
					$fontsize = $mmsize*(72/25.4); //Get size in points (pt)
				}
				else
				{
					$v = strtoupper($v);
					...
					//Values obtained from http://www.w3schools.com/html/html_reference.asp
						case 'XX-SMALL':	$fontsize = (0.7)* 11;		break;
						case 'X-SMALL':		$fontsize = (0.77) * 11;	break;
						case 'SMALL':		$fontsize = (0.86)* 11;		break;
						case 'MEDIUM':		$fontsize = 11;				break;
						case 'LARGE':		$fontsize = (1.2)*11;		break;
						case 'X-LARGE':		$fontsize = (1.5)*11;		break;
						case 'XX-LARGE':	$fontsize = 2*11;			break;
					}
				}
				$this->cell[$this->row][$this->col]['fontsize']	= $fontsize;
			}
		...
		case 'HR': //Default values: width=100% align=center color=gray
			...
			if(isset($attr['WIDTH']) && $attr['WIDTH'] != '') $hrwidth = ConvertSize($attr['WIDTH'],$this->pgwidth);
			if(isset($attr['ALIGN']) && $attr['ALIGN'] != '') $hralign = $align[strtolower($attr['ALIGN'])];
			if(isset($attr['COLOR']) && $attr['COLOR'] != '') $hrcolor = ConvertColor($attr['COLOR']);
...
//! @return void
	...
	if($tag=='SPAN')
	{
		if(!$this->pbegin and !$this->divbegin and !$this->tablestart)
		{
			...
			if($this->position_on)
			{
				$this->x = $this->position_tmp_x;
				$this->y = $this->position_tmp_y;
			}
			//Reset values
			$this->Reset();
		}
	...
	switch($k){
			...
			case 'FONT-SIZE': //Does not support: smaller, larger
				...
				else
				{
					...
					{
					//Values obtained from http://www.w3schools.com/html/html_reference.asp
						case 'XX-SMALL':	$this->SetFontSize( (0.7)* 11);		break;
						case 'X-SMALL':		$this->SetFontSize( (0.77) * 11);	break;
						case 'SMALL':		$this->SetFontSize( (0.86)* 11);	break;
						case 'MEDIUM':		$this->SetFontSize(11);				break;
						case 'LARGE':		$this->SetFontSize( (1.2)*11);		break;
						case 'X-LARGE':		$this->SetFontSize( (1.5)*11);		break;
						case 'XX-LARGE':	$this->SetFontSize( 2*11);			break;
					}
				}
				$this->lineheight = (5*$this->FontSizePt)/11;
			break;
...
$this->strike = true;
			...
			$this->SetStyle('U',true);
				...
				case 'COLOR': // font color
					...
					break;
				case 'POSITION':
					$this->position_tmp_x = $this->x;
					$this->position_tmp_y = $this->y;
					$this->position_on = true;
					if(strtolower($v) == 'fixed'){
						if(isset($arrayaux['LEFT']))
							$this->x=ConvertSize($arrayaux['LEFT'],$this->pgwidth);
						if(isset($arrayaux['RIGHT']))
							//$this->x=$this->pgwidth-str_replace('px', '', $arrayaux['RIGHT']);
							$this->x=ConvertSize($arrayaux['RIGHT'],$this->pgwidth);
						if(isset($arrayaux['TOP']))
							//$this->y=str_replace('px', '', $arrayaux['TOP']);
							$this->y=ConvertSize($arrayaux['TOP'],$this->pgwidth);
						if(isset($arrayaux['BOTTOM']))
							$this->y=($this->fh-ConvertSize($arrayaux['BOTTOM'],$this->pgwidth));
					}
					break;
		}//end of switch($k)
 }//end of foreach
